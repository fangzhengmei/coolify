...
Coolify **不介入**证书续期逻辑，续期完全由 Traefik 内部处理。Coolify 代码中对证书续期失败**没有任何处理或监控逻辑**，以下仅描述 Traefik 自身行为（由其源码和 ACME 协议决定）：

| 阶段 | Traefik 行为 | Coolify 代码是否感知 |
|-----|-------------|-------------------|
| 证书有效期内 | 正常服务 HTTPS | ❌ 不感知 |
| 到期前 30 天 | Traefik 自动发起续期（ACME 协议行为） | ❌ 不感知 |
| 续期成功 | 新证书写入 `acme.json`，热替换 | ❌ 不感知 |
| 续期失败 | Traefik 重试（每次路由请求时尝试），旧证书继续使用 | ❌ 不感知，不记录日志 |
| 证书过期后 | TLS 握手使用过期证书，客户端自行决定是否接受 | ❌ 不感知 |

**代码可确认的续期失败原因**（从 Traefik 配置反推）：
1. `httpchallenge.entrypoint=http` 要求 80 端口可达 — 若被防火墙拦截则挑战失败
2. 域名 DNS 未指向本机 — Let's Encrypt 无法到达
3. Cloudflare 代理开启且 SSL 模式不当 — 干扰 HTTP-01 挑战
4. `acme.json` 文件权限错误 — Traefik 无法写入新证书

**代码中的缺失**：Coolify 没有对 `acme.json` 有效性、证书到期时间、续期失败的任何监控。要排查续期问题，只能直接查看 Traefik 容器日志：
```bash
